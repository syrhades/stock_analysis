...
# source("~/sdcard/syrhadestock/R/cl_part_function.R")
...
frm_repair_to_xts<-function(df_obj){
    df_spec_stock_xts<-transform(df_obj,Adjusted= amount/vol/4)
    colnames(df_spec_stock_xts)[grep("vol",colnames(df_spec_stock_xts))]<-"Volume"
    colnames(df_spec_stock_xts)[grep("date",colnames(df_spec_stock_xts))]<-"Date"
    df_spec_stock_xts <- xts(df_spec_stock_xts[,-1],df_spec_stock_xts[,1])
    df_spec_stock_xts %>% return
    }
...
to.weekly(GS) %>% chartSeries()
to.monthly(GS) %>% chartSeries()
...
frm_num_2_Date <- function(x){
  if (!is.character(x)){ # not char
    char_date <- x%>%as.character
  }else{char_date <- x }
 as.Date(char_date,"%Y%m%d") %>% return
}

# done_510160.txt
df_spec_stock<-frm_stock_from_tdx_file(000001)
df_spec_stock<-frm_stock_from_tdx_file(510160)
# date format from int to Date 
...
# ,on=NA,col=6)
addTA(df_spec_stock_xts[,"Close"] %>% frm_syr_bpctB
,on=NA,col=5)
addMACD()
...
l<- s<0     #method 1
l_h<-s>100  #method 2
l_3<-s$pctB > quantile(s$pctB,probs=0.95,na.rm = T) #method 3

c<-df_month[,"..Close"]
l_close<-c >= quantile(c,probs=0.95,na.rm = T)
df_month%>% chartSeries()
# addTA(l_3,on=-1,col='green',border='grey')
addTA(l_3&l_close,on=-1,col='green',border='grey')
...
autoplot(m, which = 1:6, colour ='dodgerblue3',smooth.colour ='black', smooth.linetype ='dashed',ad.colour ='blue',label.size = 3, label.n = 5, label.colour ='blue',ncol = 3)
...
also installing the dependencies ‘PerformanceAnalytics’, ‘  ’, ‘FinancialInstrument’

install.packages("blotter", repos="http://R-Forge.R-project.org") 
install.packages("quanstrat", repos="http://R-Forge.R-project.org") 
...
ret.var = "ret.0.1.m", by.var = "sector",
by.period = FALSE)
counts(bt)
...
bt <- backtest(starmine, in.var = "smi",
buckets = 4, ret.var = c("ret.0.1.m",
"ret.0.6.m"), by.period = FALSE)
...
#######################################################
# sample dygraph for price and some bband bandwidth
#######################################################

cbind(df_spec_stock_xts[,4],
      frm_syr_bpctB(df_spec_stock_xts[,4])$bandwidth) %>% dygraph


dygraph(presidents, main ="Presidential Approval") %>%
dyAxis("y", label = "Temp (F)",valueRange =c(0, 100)) %>%
...
dyAxis("x", label = "Year",axisLabelColor="red") %>%
...
# Add a vertical event line to a dygraph  垂直
...
dyHighlight(highlightCircleSize = 15,
...
#   dygraph limit line  important  水平
dygraph(presidents, main = "Presidential Approval") %>%
dyAxis("y", valueRange = c(0, 100)) %>%
dyLimit(min(presidents, na.rm = TRUE), "Min",
# strokePattern = "solid", color = "blue")
strokePattern = "dotdash", color = "blue")%>%
dyLimit(max(presidents, na.rm = TRUE), "Max",
    strokePattern = "solid", color = "blue",labelLoc="right") %>%
dyLimit(79, "95",
    strokePattern = "dotted", color = "red",labelLoc="right") %>%
dyLimit(quantile(presidents,0.5,na.rm=T) %>% as.numeric, "50",
    strokePattern = "solid", color = "rgb(255,100,200)",labelLoc="right")
#######################################################################
...
connectSeparatedPoints = FALSE, strokeWidth = 1, strokePattern
strokeBorderWidth = NULL, strokeBorderColor = "white", plotter
...
axisLineColor = "black", axisLineWidth = 0.3, axisLabelColor =
...
mobileDisableYTouch = TRUE, timingName = NULL, useDataTimezone
retainDateWindow = FALSE)

#########import sample range selector###########################################3
...
lungDeaths <- cbind(ldeaths, mdeaths, fdeaths)
dygraph(lungDeaths, main = "Deaths from Lung Disease (UK)") %>%
dySeries("mdeaths", drawPoints = TRUE, color = "blue",fillGraph=T) %>%
# dySeries("fdeaths", stepPlot = TRUE, color = "red")
dySeries("fdeaths", stemPlot = TRUE, color = "red")
...
dySeries("mdeaths", drawPoints = TRUE, color = "blue",fillGraph=T) %>%
# dySeries("fdeaths", stepPlot = TRUE, color = "red")
dySeriesData(name="carlos", values=fdeaths)

#### important display shade
dygraph(nhtemp, main = "New Haven Temperatures") %>%
dyShading(from = "1920-1-1", to = "1930-1-1") %>%
dyShading(from = "1940-1-1", to = "1950-1-1")

tttx<-dygraph(nhtemp, main = "New Haven Temperatures") %>%
dyShading(from = "48", to = "52", axis = "y") %>%
dyShading(from = "50", to = "50.1", axis = "y", color = "black")

### test css
dygraph(nhtemp, main = "New Haven Temperatures") %>%
  dyCSS("/media/syrhades/DAA4BB34A4BB11CD/test.css")
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
/media/syrhades/DAA4BB34A4BB11CD/test.css
.dygraph-title {
  color: red;
  font-weight: bold;
}
...
dygraph(ret, main = "Microsoft Share Price") %>% 
  ...
  dyShading(from = mn - std, to = mn + std, axis = "y",color= "red")
