	...
	recursive_data = function(n,all_report,call_function){
		...
		## call_function = rbind or cbind
	       if ((n == 0) | (n == 1)){
			...
			sum = call_function(recursive_data(n-1,all_report,call_function),all_report[[n]])
		return (sum)
	}
...
frm_show_delta_df<-function(df_obj,position_idx){
	df_obj[,position_idx]<-as.numeric(df_obj[,position_idx])
	df_indicator<-df_obj[order(df_obj[,1]),] 
	df_indicator$percent<-lag(diff(df_indicator[,position_idx])/df_indicator[,position_idx]*100)
	# return(df_indicator)
	p = ggplot(df_indicator, aes(x = V1, y = percent,  group = factor(1)))
	p +geom_bar(stat = "identity")
}
dftest<-ta$query_df[,c("V1","V99")]
dftest<-dftest[-1,]
frm_show_delta_df(dftest,2)
