...
library(sqldf)
dfsql<-read.csv.sql("/tmp/done_000001.txt",sql="select * from file where Open >8.5")
...
where d1.g = g2 and v
identical(a12r, a13s)
two select statements to one using a subquery
g, t, v
as g2, avg(v) as avg_v from DF group by g)
> avg_v")10
sqldf
...
all.equal(Sort(a19s), Sort(a19r), check.attributes = FALSE)sqldf
11
...
STM05-2 2005/02/28
"
Date.Time LocationQuality Latitude Longitude
17:35 Good -35.562 177.158
19:44 Good -35.487 177.129
23:01 Unknown -35.399 177.064
07:28 Unknown -34.978 177.268
18:06 Poor -34.799 177.027
18:47 Poor -34.85 177.059
12:49 Good -35.928 177.328
21:23 Poor -35.926 177.314
