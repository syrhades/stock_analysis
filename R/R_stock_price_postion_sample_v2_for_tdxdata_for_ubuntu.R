...
func.test_stock<- function(dataframe){
	...
	current_price<-tail(close$close,1)
	#cat (current_price)
	all<-unlist(unlist(summary(close$close)))
	#print(class(all))
	#print(all)
	#print(current_price[[1]])
	all<-apply(all,1,function(x){gsub(".*:","",x)})
	all<-apply(all,1,function(x){gsub("[[:space:]]*$","",x)})

	#price1<-gsub("^.*:","",)
	#print(all[8])
	#print(all[[11]])
	#return (all)
	if (current_price < all[[8]]) return("b")
	if (current_price > all[[11]]) return("s")
...
apply(fx_seriesdata[1,],1,s_Func,c1='No')


bstable<-s_return_bs_point(fx_seriesdata[1,])
bstable
...
gzmt<-read.table("~/sdcard1/stock_data/done_000001.txt",header=TRUE,sep=",")
...
fx_seriesdata<-as.data.frame(seriesdata)
...
time1<-Sys.time()
bstable<-s_return_bs_point(fx_seriesdata)
f(time1)



#BSpoint<-apply(fx_seriesdata,1,s_Func,c1='No')
...
g<-ggplot(aes(x=Date, y=Close),data=fortify(BS_table,melt=TRUE))
g<-g+geom_line()
g<-g+geom_line(aes(x=Index, y=Value,colour=Series),data=fortify(cdata,melt=TRUE))
g<-g+geom_point(aes(x=Date,y=Close,colour=Type),data=fortify(BS_table,melt=TRUE))
g
	g<-g+geom_line(aes(colour=Series),data=fortify(ldata[,-1],melt=TRUE))
	...
	g
#  simulate trade
BS_table<-subset(bstable,Type!="w")
tb1<-my_trade(bstable)
