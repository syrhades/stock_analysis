...
cl_Stock_report_process <- R6Class("Stock_report_process",    # 定义一个R6类
	...
	frm_single_table = function(positon){
			...
			#cat(url)
			url_report<-paste("http://stockdata.stock.hexun.com/2008en/",self$report_type[positon],".aspx?stockid=",self$stockid,sep="");
			cat(url_report)
			year_list=self$frm_get_finance_year_list(url_report)
			#cat(year_list)
			#return(year_list)
			...
			f_report_cn<-cl_report_combine$new(self$stockmarket)$single_table(all_reports_cn)%>%as.data.frame;
...
cl_report_combine <- R6Class("Stock_report_combine",    # 定义一个R6类
	...
	single_table = function(all_reports_cn){
		n_year<- switch(
					self$stock_market,
...
lapply(seq(1,4),tb$test_lappy,1,re_df_4) 
    ...
    dbfile<-paste(frm_set_path(),"tb_finance4.db",sep="/")
