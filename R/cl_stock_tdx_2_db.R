...
cl_stock_data_tool <- R6Class("stock_data",    # 定义一个R6类
	...
	public=list(
		...
		# if(str_detect(R_PLATFORM, "linux")) path<-"~/sdcard"
		if(str_detect(R_PLATFORM, "linux")) path<-"/media/syrhades/新加卷"
		else path<-"d:"
...
# system.time(frm_batch_stock_report(stockfile=stockfile,dbf_name=dbfile))


##########################################
# regular extract
# stringr::str_extract(v_file_list[1],"[:digit:]{6}")


v_file_list <- system("ls  ~/sdcard/stock_data/",intern =T)
v_file_list <- v_file_list %>% laply(function(x) paste0("~/sdcard/stock_data/",x))

frm_gen_pricedf <- function(x) {
	stockid <- stringr::str_extract(x,"[:digit:]{6}")
	df_price <- read.csv (x)
	df_price$stockid <- stockid
	df_price %>% return
}
df_stock_price <- v_file_list %>% ldply(frm_gen_pricedf)
ta<-cl_db_oprate$new("/media/syrhades/新加卷/tb_stocknew.db")
ta$frm_save_fin_table2_sqlite("/media/syrhades/新加卷/tb_stocknew.db",
			R_df = df_stock_price,
			tb_name = "stockprice")
...
# system.time(frm_batch_stock_report(stockfile=stockfile,dbf_name=Args[7]))
#library(profvis)
