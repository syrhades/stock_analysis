library(RODBC)
myconn<-odbcConnect("bis",rows_at_time = 10)
...
system.time(apply(alltable,1,main,myconn,"syrhades"))

tn<-as.character(alltable$name)
sapply(tn[1:3],main,myconn,"SEEBURGER")

myconn<-odbcConnect("bis",rows_at_time = 3)
system.time(sapply(tn[1:10],main,myconn,"SEEBURGER"))
sapply(tn[4:6],main,myconn,"SEEBURGER")


#test part using for loop
frm_test<-function(){
	close(myconn)
