...
# generate BS point 
# set trend up ,down function
Series<-apply(ldata,1,myFUN,c1='ma5',c2='ma20')
pdata2<-myFUN_set_trend(Series,ldata)
...
# g+geom_rect(data=data.frame(xmin=250, xmax=300, ymin=0.0025, ymax=Inf), aes(xmin=xmin, xmax=xmax, ymin=ymin,ymax=ymax), fill="yellow", alpha=0.5) 

# g<-g+geom_rect(data=data.frame(xmin=250, xmax=300, ymin=0.0025, ymax=Inf), aes(xmin=xmin, xmax=xmax, ymin=ymin,ymax=ymax), fill="blue", alpha=0.5)
# g
...
#    g<-g+geom_line(aes(colour=Series),data=fortify(ldata[,-1],melt=TRUE))
#     # g<-g+geom_rect(aes(NULL, NULL,xmin=start,xmax=end,fill=plan),ymin = yrng[1], ymax = yrng[2],data=plan)
#    g<g+geom_rect(data=data.frame(xmin=as.Date("2013-01-01"), xmax=as.Date("2014-01-01"), ymin=0, ymax=Inf), aes(xmin=xmin, xmax=xmax, ymin=ymin,ymax=ymax), fill="yellow", alpha=0.5) 
# g<-g+geom_rect(aes(NULL, NULL,xmin=as.Date("2016-06-01"),xmax=as.Date("2016-06-20")),data=ldata)

# http://data.eastmoney.com/xuangu/#Yz1bZ2J6YjA4KDR8MC4zKV1bZ3p6YjAyKDJ8MSldfHM9Z3p6YjAyKDJ8MSl8c3Q9LTE=

# http://data.eastmoney.com/bbsj/201512/xjfh.html

# http://data.eastmoney.com/zlsj/sb.html