...
frm_sub_lapply<-function(iter_obj){
	function(FUN1,FUN2) {
		llply(iter_obj,FUN1)  %>% llply(FUN2)
	}
}

frm_show_report_reprocess <- function(df_report){
	# convert encoding utf-8
	df_report [1,] <- sapply(df_report [1,],function(x) {Encoding(x) <- "UTF-8";return(x)}) # change to Chinese utf-8 format
	# colnames(df_report) <- df_report[1,]
	# df_report[-1,] %>% return
	event<-df_report[,1] %>% stringr::str_detect("12\\.31") # filter year data
	event[1]<-TRUE
	df_report[event,]
}

frm_filter_vol_name <- function(fliter_word){
	function(filter_obj){
		result <- filter_obj %>% stringr::str_detect(fliter_word)
		return(result)
		# function(output_FUN,...){
		# 	result %>% output_FUN(...)
		# }
...
frm_filter_vol_name2 <- function(fliter_word){
	function(filter_obj){
		result <- filter_obj %>% stringr::str_detect(fliter_word)
...
frm_filter_stockid<-function(db_list,stockid){
	
	db_list[db_list%>% stringr::str_detect(stockid)]
}


write_file <- partial(write.table, sep="\t",append=T,)
