...
<bytecode: 0x000000000c8e5fe8>
<environment: namespace:stats>



> getS3method("monthplot","default")
function (x, labels = 1L:12L, ylab = deparse(substitute(x)),
    ...
    col.base = col, lty.base = lty, lwd.base = lwd, ...)
{
    ...
    if (!add) {
        ...
        Call$labels <- Call$times <- Call$phase <- Call$base <- Call$type <- Call$box <- Call$add <- Call$col.base <- Call$lty.base <- Call$lwd.base <
- NULL
        eval(Call)
        if (axes) {
