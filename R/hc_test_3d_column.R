...
hc_env<-environment(hc_chart)
"hc_env$hc_carlos_set2<-" <- function(hc,attr,...){
	.hc_opt(hc, attr, ...)
}
hc_env$frm_hc_set_opr<-function(hc,property,change_content){
	# hc <- hc
...
hc_env$frm_hc_get_opr<-function(hc,property,...){
...
width<- max(test_df$depth)
step_t1<- 	mapply(function(x,y) sprintf("hc_add_series(data= %s,depth=%s,groupZPadding=%s)",x,width-y,y),test_df$value,test_df$depth)%>% 
			llply(function(x) x)
